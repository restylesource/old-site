...
class widget_submit extends WP_Widget {
	...
	function widget_submit() {
		...
		$this->WP_Widget( 'widget_cosmo_submit' , _TN_ . ' : ' . __( 'Submit Content' , 'cosmotheme' ) , $widget_ops );
	}
		...
		if(is_numeric(options::get_value( 'upload' , 'post_item_page' )) && options::get_value( 'upload' , 'post_item_page' ) > 0){
			...
			if( options::logic( 'general' , 'user_register' ) ){
				if( is_user_logged_in () ){
					?><p><a href="<?php  echo get_page_link(options::get_value( 'upload' , 'post_item_page' ));  ?>"><img src="<?php echo get_template_directory_uri() ?>/images/submit.bg.png" alt="<?php echo $title ?>" /><?php echo $title ?></a></p><?php
				}else{
					?><p><a href="javascript:void(0)" onclick="get_login_box('')"><img src="<?php echo get_template_directory_uri() ?>/images/submit.bg.png" alt="<?php echo $title ?>" /><?php echo $title ?></a></p><?php
				}
			}else{
