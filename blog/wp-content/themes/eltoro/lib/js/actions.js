...
act.post_relation = function( post_id , meta_label , field_id ){
    jQuery( document ).ready(function(){
        jQuery.post( MyAjax.ajaxurl , {"action" : 'post_relation' , "post_id" : post_id , "meta_label": meta_label , "field_id" : field_id} , function( result ){jQuery( 'span#' + field_id  ).html( result );jQuery('div.' + field_id ).show();} );
    });
}
...
act.preview = function( family , size , weight , text , selector ){
	jQuery( document ).ready(function(){
		jQuery.post( MyAjax.ajaxurl , {"action" : "text_preview" , "family" : family , "size" : size , "weight" : weight , "text" : text} , function( result ) {jQuery( selector ).html( result );} );
	});
}
...
act.like_ajax = function( post_id, action, selector ){
    if(action == 'like'){
        var  current_selector = jQuery('.love-'+post_id);
        var  current_anti_selector = jQuery('.hate-'+post_id);
    }else if(action == 'hate'){
        var  current_selector = jQuery('.hate-'+post_id);
        var  current_anti_selector = jQuery('.love-'+post_id);
    }
    ...
    jQuery(function(){
        jQuery.post( 
            ...
            function( result ) {
                ...
                jQuery('strong.like-' + post_id).html(json['likes']);
                jQuery('strong.hate-' + post_id).html(json['hates']);
                
                current_selector.addClass('active');
                current_anti_selector.removeClass('active');
                
                var total_nr_votes = json['likes'] + json['hates'];
                var votes_label = 'votes';
                if(total_nr_votes == 1){
                    votes_label = 'vote';
                }else{
                    votes_label = 'votes';
...
jQuery( function(){
    ...
    var menu_text_color = jQuery( '#pick_menu_text_color' ).val();
    jQuery( '.generic-header_type' ).change( function(){
        ...
        jQuery( '.header-preview nav.colored, .nav-container .container, .cosmo-icons.colored' ).find( 'a' ).css( 'color' , menu_text_color );
    ...
    jQuery( '.generic-menu_type' ).change( function(){
        ...
        var _new = false;
        if( val.indexOf( 'colored' ) == -1 ){
            jQuery( '#pick_menu_color' ).parent().parent().slideUp();
            jQuery( '#pick_menu_text_color' ).parent().parent().slideUp();
        }else{
            jQuery( '#pick_menu_color' ).parent().parent().slideDown();
            jQuery( '#pick_menu_text_color' ).parent().parent().slideDown();
        }
    ...
    jQuery( '#link_pick_menu_text_color' ).click( function(){
        jQuery( '#colorPickerDiv_menu_text_color div.farbtastic' ).unbind( 'mouseout mousemove' ).on( 'mouseout mousemove', function(){
            menu_text_color = jQuery( '#link_pick_menu_text_color' ).css( 'background-color' );
            jQuery( '.header-preview nav.colored, .nav-container .container, .cosmo-icons.colored' ).find( 'a' ).css( 'color' , menu_text_color );
        });
    });
...
function load_more(obj, view, type, id){
    ...
    jQuery.ajax({
        url: MyAjax.ajaxurl,
        data: '&action=load_more&current_page='+current_page+'&getMoreNonce='+MyAjax.getMoreNonce+'&view='+view+'&type='+type+'&id='+id,
        type: 'POST',
        cache: false,
        success: function (data) { 
            json = eval("(" + data + ")");
