<?php
	...
	if( is_search() ){
		...
		if( $wp_rewrite->using_permalinks() ){
				$pagination['base'] = user_trailingslashit( trailingslashit( remove_query_arg( 'fp_element' , remove_query_arg( 'type' , remove_query_arg( 's', get_pagenum_link( 1 ) ) ) ) ) . 'page/%#%/', 'paged' );
        }
        ...
        if( !empty( $wp_query->query_vars['fp_element'] ) ){
				$pagination['add_args'] = array( 'fp_element' => get_query_var( 'fp_element' ) );
        }
