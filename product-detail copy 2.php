...
*/		
		...
		// SEARCH SOURCES CARRY this Product

		$source_result = source_search_by_product($_GET['id'], $_REQUEST['search_state'], 1);
		...
		if($source_result){
			...
			while($source_row = mysql_fetch_array($source_result)){
				...
				$carry_output.= '<li class="relative">
									...
									<img src="/assets/images/icon-product-stock.jpg" class="icons left" /> 	</div> 								
									<div class="hover left">
			...
			//if($source_count<3 && $state_location!=""){
			if($source_count<3){
			
				$i = 0;
			
				$source_result = source_search_by_product($_GET['id'], '', 1);
				...
				while($source_row = mysql_fetch_array($source_result)){
					...
					if(!in_array($source_row[0], $source_array)){
						$outside_output.= '<li>
											...
											<div class="hover relative left"> <div class="tag prod-order canorder">Carries Product Line / Can Order</div> <!-- end tag -->
											<img src="/assets/images/icon-product-line.jpg" class="icons left" /> </div>
											<div class="hover relative left"> <div class="tag prod-order canorderemail">Email Source</div> <!-- end tag --><a href="/ajax/ajax-product-email.php?product_id=' . $_GET['id'] . '&source_id=' . $source_row[0] . '" class="email">Email us</a></div>
		...
		// SEARCH SOURCES CARRY this look
		$source_result = source_search_by_product($_GET['id'], $state_location, 0);
		...
		if($source_result){
			...
			while($source_row = mysql_fetch_array($source_result)){
				...
				$look_output.= '<li>
									...
									<div class="hover relative left"><div class="tag prod-order canorder">Carries Product Line / Can Order</div> <!-- end tag -->
									<img src="/assets/images/icon-product-line.jpg" class="icons left" /> </div> 
									<div class="hover relative left"><div class="tag prod-order canorderemail">Email Source</div> <!-- end tag --> <a href="/ajax/ajax-product-email.php?product_id=' . $_GET['id'] . '&source_id=' . $source_row[0] . '" class="email">Email us</a>
									</div>
								</li>';
				$i++;
			}
		...
		// SEARCH SOURCES WHO CAN HELP WITH LOOK
		$source_result = source_search_by_product($_GET['id'], $state_location, 2);
		...
		if($source_result){
			...
			while($source_row = mysql_fetch_array($source_result)){
				...
				$source_style_output.= '<li>
											...
											<h3>' . $source_row['company'] . '</h3>
											<h4>' . $source_row['city'] . ', ' . $source_row['state'] . '</h4>
											 <div class="hover relative left"><div class="tag prod-style getthestyle">Get The Style</div> <!-- end tag -->
								 			<img src="/assets/images/icon-product-style.jpg" class="icons left" /></div>
											
											 <div class="hover relative left"><div class="tag prod-style emailstyle">Email Source</div> <!-- end tag --> <a href="/ajax/ajax-product-email.php?product_id=' . $_GET['id'] . '&source_id=' . $source_row[0] . '" class="email">Email us</a></div><div class="clearfix"></div><!-- Clear -->
										</li>';
				$i++;
			}
		}
...
<br clear="all"><br />
		...
		<section id="get-the-look">
			<h2><span>Sources Who Can Help You</span> <a href="<?=$change_location_link?>" class="change-location">Change location ></a>
            </h2> 
