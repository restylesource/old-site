<?
	...
	$aColumns = array( 'product_id', 'product', 'product', 'upc', 'msrp', 'status', 'keywords', 'short_description', 'long_description', 'tbl_product.user_id' );
	...
	$sWhere = "WHERE archived_ind=0 AND product_id IN (SELECT product_id FROM tbl_retailer_inventory WHERE user_id = " . $g_sess->get_var("user") . ")";
	...
	while ( $aRow = mysql_fetch_array( $rResult ) ){
		...
		$edit_link = ($g_sess->get_var("user") == $aRow['user_id'] || $g_sess->get_var("systemTeam") != "retailer" ) ? "products-new.php" : "products-inventory-add.php";
		
		//mail('kevin@atekie.com', $g_sess->get_var("user") . ' : ' . $aRow['user_id'] . " : " . $g_sess->get_var("systemTeam"), $edit_link, 'from: support@restylesource.info');
		...
		$row[0] = '<input type="checkbox" name="product_delete[]" value="' . $aRow['product_id'] . '" />';
		...
		$row[4] = $aRow['upc'] ;
		$row[5] = $aRow['status'] ;
		$row[6] = '<a rel="Edit Product" href="' . $edit_link . '?product_id=' . $aRow['product_id'] . '" class=""><img src="gfx/icon-edit.png" alt="edit" /></a>
